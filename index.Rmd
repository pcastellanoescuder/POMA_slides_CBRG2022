...
.center[
.textboxorange[All `POMA` preprocessing methods return a preprocessed `SummarizedExperiment` object]
]
...
```{r, echo = FALSE}
clean_object <- st000284 %>% 
  PomaNorm(method = "log_scaling")

clean_object <- clean_object[, clean_object$group %in% c("CRC", "Healthy")]
```
...
```{r, warning = FALSE, comment = FALSE, message = FALSE, fig.height = 3.5}
clean_object %>%
  PomaBoxplots(group = "features", jitter = TRUE, legend_position = "right",
               feature_name = c("glyceraldehyde", "lysine", "linolenic_acid")) #<<
```
